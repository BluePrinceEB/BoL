...
local UPDATE_PATH = "/BluePrinceEB/BoL/master/Quinn.lua".."?rand="..math.random(1,10000)
local UPDATE_FILE_PATH = SCRIPT_PATH..GetCurrentEnv().FILE_NAME
local UPDATE_URL = "https://"..UPDATE_HOST..UPDATE_PATH

if AUTOUPDATE then
	local ServerData = GetWebResult(UPDATE_HOST,"/BluePrinceEB/BoL/master/Quinn.version")
	if ServerData then
...
function Quinn:Variables()
	self.Q  = { speed = 1550, delay = 0.25, range = 1000, width = 90, collision = true, aoe = false, type = "linear" }
	self.W  = { range = 2100 }
	self.E  = { range =  700 }
	self.R  = { range =  550 }
